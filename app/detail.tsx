...
export default function DetailScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setProviders(byRegion?.flatrate ?? []); // MVP: subscription providers
      } catch (e: any) {
  ...
  const regionPicker = useMemo(() => {
    // MVP: basit 3 seçenek. Sonra dropdown yaparız.
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 28 }}>
          <View style={{ flexDirection: "row", gap: 12 }}>
            <View style={{ width: 120, height: 180, borderRadius: 14, overflow: "hidden", backgroundColor: "#f3f4f6" }}>
              {posterUrl(item.poster_path, "w342") ? (
                <Image source={{ uri: posterUrl(item.poster_path, "w342")! }} style={{ width: "100%", height: "100%" }} />
              ) : null}
            </View>

            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 22, fontWeight: "900" }}>{title}</Text>
              <Text style={{ opacity: 0.7, marginTop: 6 }}>⭐ {item.vote_average?.toFixed(1)}</Text>
          ...
          <Text style={{ marginTop: 20, fontSize: 18, fontWeight: "900" }}>Where to watch</Text>
          <Text style={{ opacity: 0.65, marginTop: 4 }}>
            Showing subscription providers for {region}
          </Text>

          {providers.length === 0 ? (
            <Text style={{ marginTop: 10, opacity: 0.75 }}>
              No subscription providers found for this region.
            </Text>
          ) : (
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 12 }}>
              {providers.map((p) => {
                ...
                return (
                  ...
                  >
                    <View style={{ width: 36, height: 36, borderRadius: 10, overflow: "hidden", backgroundColor: "#f3f4f6" }}>
                      {logo ? <Image source={{ uri: logo }} style={{ width: "100%", height: "100%" }} /> : null}
                    </View>
                    <Text numberOfLines={2} style={{ fontSize: 11, textAlign: "center", fontWeight: "700" }}>
                      {p.provider_name}
                    </Text>
                  </View>
                );
          ...
          {providerLink ? (
            <Text style={{ marginTop: 12, opacity: 0.6 }}>
              TMDB link available (we can add “Open” button next).
            </Text>
          ) : null}
        </ScrollView>
      )}
